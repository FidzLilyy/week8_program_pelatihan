...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO peserta (nama, email, jk, alamat, telp) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$nama, $email, $jk, $alamat, $telp]);
...
<body>
    ...
    <form action="tambah_peserta.php" method="POST">
        ...
        <select name="jenis_kelamin" id="jenis_kelamin">
            <option name="laki" value="Laki-Laki">Laki-Laki</option>
            <option nama="perempuan" value="Perempuan">Perempuan</option>
        </select>
        <label>Alamat:</label>
            <input type="text" name="alamat" required><br>
        <label>No Telepon:</label>
            <input type="tel" name="telp" pattern="\+?[0-9]{1,4}?[-. ]?(\(?\d{1,3}?\))?[-. ]?\d{1,4}[-. ]?\d{1,4}[-. ]?\d{1,9}" required><br>
